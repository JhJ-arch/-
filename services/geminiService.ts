...
3.  **문제 (questions):**
    ...
    *   **[매우 중요] '${QuestionTypes.APPLICATION}' (또는 '전이') 문제 생성 규칙 (사용자 예시 기반 강화):**
        ...
        *   **'questionText' 필드 및 <보기> 작성 규칙:**
            ...
            *   **[절대 규칙] 지시문 자체에 '<보기>'라는 단어를 사용하지 마세요.** 대신, <보기> 안의 상황을 직접 언급하여 질문을 완성하세요. (예: "A의 행동을 가장 잘 설명하는 것은?" O, "<보기>의 상황에 대한 설명으로 옳은 것은?" X)
...
export const factCheckParagraph = async (paragraphContent: string): Promise<string> => {
    const prompt = `
다음 문단의 내용에 대한 팩트체크를 수행하고, 부정확하거나 오해의 소지가 있는 정보가 있는지 검증해주세요.

[검증 대상 문단]
${paragraphContent}

**요청:**
1.  문단 내용의 사실관계를 확인해주세요.
2.  만약 부정확한 정보가 있다면, 어떤 부분이 왜 틀렸는지 설명하고 올바른 정보로 수정해주세요.
3.  모든 주장에 대한 근거와 출처를 명확히 밝혀주세요.
4.  검증 결과, 내용에 문제가 없다면 "검증 결과, 해당 문단의 내용은 사실에 부합합니다." 라고 답변해주세요.
5.  답변은 한국어로 작성해주세요.
`;
